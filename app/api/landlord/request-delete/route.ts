...
export async function POST() {
  try {
    ...
    if (session.user.role !== 'LANDLORD') {
      return NextResponse.json({ error: 'Only landlords can request deletion' }, { status: 403 });
    }
...
export async function DELETE() {
  try {
    ...
    if (session.user.role !== 'LANDLORD') {
      return NextResponse.json({ error: 'Only landlords can cancel deletion request' }, { status: 403 });
    }

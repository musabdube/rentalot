...
import {
  Users, MapPin, DollarSign, Calendar, CheckCircle, MessageSquare,
  Building2,
} from 'lucide-react';
    ...
    <div className="min-h-screen bg-gray-50">
        ...
        <div className="max-w-5xl mx-auto flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            <h1 className="text-2xl font-bold text-gray-900 flex items-center gap-2">
              <Users className="w-6 h-6 text-emerald-600" /> Tenant Roommate Listings
            </h1>
            <p className="text-sm text-gray-500 mt-1">Find tenants interested in co-renting your property</p>
          </div>
          <div className="flex gap-3">
            <Link
              href="/landlord/messages"
              className="inline-flex items-center gap-2 border border-gray-300 text-gray-700 px-4 py-2.5 rounded-full text-sm font-semibold hover:bg-gray-50 transition-colors"
            >
              <MessageSquare className="w-4 h-4" /> Messages
            </Link>
            <Link
              href="/landlord/properties"
              className="inline-flex items-center gap-2 border border-emerald-300 text-emerald-700 px-4 py-2.5 rounded-full text-sm font-semibold hover:bg-emerald-50 transition-colors"
            >
      ...
      <div className="max-w-5xl mx-auto px-4 pb-16">
        {loadingListings ? (
          ...
          <div className="text-center py-16 text-gray-400">No approved roommate listings found.</div>
        ) : (
            ...
            {listings.map((l) => (
              <div key={l.id} className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
                ...
                <div className="flex flex-wrap gap-1.5 mb-4">
                  {l.pets && <span className="text-xs bg-blue-50 text-blue-700 px-2 py-0.5 rounded-full">Pets</span>}
                ...
                {messagingId === l.id ? (
                  <div className="border-t border-gray-100 pt-3">
                    <textarea
                      ...
                      placeholder="Tell this tenant about your available property..."
                  ...
                  >
                    <MessageSquare className="w-4 h-4" /> Contact Tenant
                  </button>
                )}
              </div>
            ))}
